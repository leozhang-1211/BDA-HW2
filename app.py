...
import datetime
import google.generativeai as genai # 引入 Gemini AI 套件
...
try:
    ...
    st.sidebar.markdown("---")
    st.sidebar.header("🤖 AI 分析設定 (Bonus)")
    # 讓使用者輸入 API Key (輸入時會變成隱藏字元)
...
if st.button("✨ 根據最新數據生成 AI 分析報告"):
    with st.spinner("AI 量化分析師正在解讀市場數據中..."):
        try:
            # 【關鍵修改】：從 Streamlit 的機密環境變數中自動讀取 API Key
